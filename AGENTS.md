...
| `packs/code-audit/` | `0.2.1` | 4 | 2 | 存量代码 bug 审查 family：audit-planner / audit-reviewer / audit-verifier / audit-reporter + 双 agent；按模块切分、双 agent 串行确认、输出 HTML（必需）+ Excel（可选）报告；**v0.2.0：audit-planner 先识别项目语言+架构（C/C++ embedded SOA / Python web / frontend SPA / generic 等）→ 推荐针对性 review_checklist 与用户握手确认 → 切模块；finding.category 由 plan.json review_checklist 动态决定（不再固定 11 类）；report header 展示 profile + checklist；旧 plan.json 缺字段时回退 base 11 完全兼容**。**v0.2.1：强化握手契约真正落地——agent.md/SKILL.md 描述加 ⛔ CRITICAL callout 把"必须停下等用户回复"提到顶部, 新增 `audit-planner/references/handshake-protocol.md` 治"LLM 把握手当文档读"根因, audit-reviewer 加 Handshake Re-Gate 二次闸门（plan.json 到 reviewer 时若 user_confirmed=false 不直接扫, 先再做一轮 confirm/del/add/edit/restart-planner/skip-and-warn 握手）** |
