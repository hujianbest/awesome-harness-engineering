...
合计 4 个 pack × 38 个 skill × 3 个宿主 = `garage init --hosts all` 物化 114 个 skill 文件 + 6 个 agent 文件（3 agent × 2 hosts；agent 仅装到 claude / opencode）。
...
F008 cycle 把 `.agents/skills/` 整个删除，改为 dogfood 安装产物作为 IDE 加载入口。**首次 clone 本仓库的贡献者**必须在仓库根跑一次 dogfood 才能在 IDE 内加载到这 38 个 skill：

```bash
cd /path/to/garage-agent
garage init --hosts cursor,claude
# → 在仓库根 dogfood 出 .cursor/skills/ + .claude/skills/，IDE 即可加载 38 个 skill
