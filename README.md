...
- **Skill packs that travel** — 4 packs ship in v0.1.0 (`garage` / `coding` / `writing` / `search`) with **38 skills + 3 production agents**. Install third-party packs from any git URL; publish your own with `garage pack publish`.
...
`garage init --hosts all` materializes **114 skill files + 6 agent files** (38 skills × 3 hosts; agents to claude + opencode only — Cursor has no agent surface yet).
